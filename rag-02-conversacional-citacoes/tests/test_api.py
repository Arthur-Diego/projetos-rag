...
#: propósito, em vez de contar com o Qdrant estar fora do ar: um teste que passa
...
PROPERTIES_SEM_QDRANT = RagProperties(
    openai_api_key="sk-teste-nao-usada", qdrant_port=59_999
)


def make_client(hits, answer=answer_with_citation, rewrite="pergunta reescrita"):
...
def test_capabilities_declara_history_e_os_cinco_parametros():
    client, _, _ = make_client(IN_CORPUS)

    body = client.get("/capabilities").json()

    assert body["project"] == "rag-02-conversacional-citacoes"
...
def test_qdrant_fora_do_ar_vira_503_com_o_comando_a_rodar():
    ...
    """
    app = create_app()
    app.dependency_overrides[deps.provide_properties] = lambda: PROPERTIES_SEM_QDRANT
    ...
    assert response.status_code == 503
    assert body["code"] == "SERVICE_UNAVAILABLE"
    assert "docker compose up -d qdrant" in body["detail"]
