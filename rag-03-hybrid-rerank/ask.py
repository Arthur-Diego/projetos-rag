...
from rag.config import DEFAULT_CANDIDATES, DEFAULT_K, DEFAULT_RRF_K
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"candidatos por caminho antes da fusao (padrao: {DEFAULT_CANDIDATES})",
    )
    parser.add_argument(
        "--rrf-k",
        type=int,
        default=DEFAULT_RRF_K,
        help=f"amortecimento da fusao RRF (padrao: {DEFAULT_RRF_K})",
    )
    parser.add_argument(
        "--sem-hibrida",
        action="store_true",
        help="desliga a busca BM25; deixa so a busca densa",
    )
    parser.add_argument(
        "--sem-rerank",
        action="store_true",
        help="desliga a reordenacao por cross-encoder",
    )
...
def main() -> int:
    ...
    try:
        ...
        facade = build_query_facade(
            ...
            rrf_k=args.rrf_k,
            hybrid=not args.sem_hibrida,
            rerank=not args.sem_rerank,
        )
