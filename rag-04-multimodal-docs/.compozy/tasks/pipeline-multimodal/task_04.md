...
- [ ] 4.1 `RetrievalService` com `require_index`, métrica por estágio e descarte de órfão
- [ ] 4.2 `PromptBuilder` com originais íntegros por `kind` e log de contexto
- [ ] 4.3 `GenerationService` (Protocol + OpenAI) com recusa e retries
- [ ] 4.4 `QueryFacade`, `ask.py`, `POST /ask` com semântica 1.3.0
- [ ] 4.5 `HealthChecker` com dessincronia e `GET /health`; `GET /capabilities`
- [ ] 4.6 Script CLI de reset (dois armazéns, preserva partição)
- [ ] 4.7 `JsonPresenter` (omissão de opcionais) e `ConsoleReporter`; `serve.py`
- [ ] 4.8 Testes do escopo fixado e validação manual da pergunta-critério
...
- [ ] T4.1 — `format_context`: hit de tabela entra com o HTML do docstore (não o excerpt); hit de texto entra com o texto; numeração 1-based preservada.
- [ ] T4.2 — Órfão: hit com `doc_id` inexistente no docstore é descartado com warning e a resposta sai 200 com os demais.
- [ ] T4.3 — `require_index`: índice vazio produz 409 antes de qualquer chamada ao gerador (fake conta chamadas).
- [ ] T4.4 — Presenter: hit `kind=texto` serializado NÃO contém a chave `content_html`; hit `kind=tabela` contém; nenhum campo `null`.
- [ ] T4.5 — Validação de borda: `k=0` e `k=21` produzem 422; `question` vazia produz 422.
- [ ] T4.6 — Health: contagens divergentes injetadas produzem `status=degraded` com evidência.
- [ ] T4.7 — Reset: após reset com armazéns fake populados, ambos vazios e segunda execução não falha.
