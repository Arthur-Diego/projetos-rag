...
Gerados a partir deste FDD em `docs/domains/rag/diagrams/mermaid/`:

- `ingestao.mmd`: fluxograma do pipeline de ingestão, organizado pelas duas fronteiras
  que mandam no custo (estágio local com cache de partição vs. estágio pago) e pela
  idempotência por `doc_id`, com roteamento por categoria e indexação dupla em ordem fixa.
- `sequencia-consulta.mmd`: sequência do `POST /ask`, da validação na borda (422 e 409
  antes de qualquer chamada paga) à resposta, mostrando a busca no Chroma sobre as
  representações, a resolução dos originais no docstore, o descarte de hit órfão e o
  prompt montado com o original íntegro.
- `dois-armazens.mmd`: visão dos dois armazéns ligados por `doc_id`, com a assimetria
  entre fonte de verdade (docstore) e índice derivado (Chroma), a ordem de escrita, o
  `/health` comparando contagens e o reset único.
- `modelo-doc-id.mmd`: modelo de dados 1 `DocumentElement` para 1 `StoredOriginal` para
  N `IndexedRepresentation`, com os atributos que vivem em cada armazém.
